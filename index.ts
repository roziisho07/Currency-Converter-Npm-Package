...
const getUserInput = async () => {
  ...
  try {
    const response = await fetch(
      `https://api.apilayer.com/exchangerates_data/convert?to=${to["To:"]}&from=${from["From:"]}&amount=${amount["amount:"]}`,
      {
        method: "GET",
        redirect: "follow",
        headers: key,
    ...
    const convertedFrom: string = data.query.from;
    const convertedTo = data.query.to;

    console.log(
      `\n${chalk.yellow("Converted")}\n${chalk.green(
        convertedFrom
      )} ${chalk.white("TO")} ${chalk.green(convertedTo)}
      \nConverted Amount: ${chalk.blue(result)}\nConversion Rate: ${chalk.blue(
        rate
      )}\n`
    );
  } catch (error) {
